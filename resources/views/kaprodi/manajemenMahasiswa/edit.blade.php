<x-app-layout>
    <x-slot name="title">Edit Data Mahasiswa - Portal Skripsi</x-slot>

    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 w-full">
        <div class="order-2 sm:order-1">
            <h3 class="font-bold text-xl text-gray-900 tracking-tight">Form Perubahan Data Mahasiswa</h3>
            <p class="text-xs text-gray-400 mt-0.5">Ubah informasi akun dan data akademik mahasiswa</p>
        </div>
    ...
    <div class="w-full bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <form action="{{ route('kaprodi.mahasiswa.update', $mahasiswa->id) }}" method="POST" class="p-8 space-y-8">
            @csrf
            @method('PUT')

            <div class="space-y-4 pt-2">
                <h4 class="text-xs font-bold uppercase tracking-wider text-blue-600 flex items-center border-b border-gray-50 pb-2">
                    <i class="fa-solid fa-key mr-2 text-sm"></i> Kredensial Akun (Akademik & Login)
                </h4>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-gray-600">Alamat Email Kampus</label>
                        <div class="relative w-full">
                            <span class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-gray-400">
                                <i class="fa-regular fa-envelope text-xs"></i>
                            </span>
                            <input type="email" name="email" value="{{ old('email', $mahasiswa->user->email) }}" required placeholder="contoh: mhs@kampus.ac.id" 
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-gray-600">Kata Sandi Baru (Password)</label>
                        <div class="relative w-full">
                            <span class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-gray-400">
                        ...
                        <p class="text-[10px] text-gray-400 mt-1">Biarkan kosong kecuali mahasiswa meminta reset password.</p>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-6 w-full">
                    <div class="space-y-1.5 w-full">
                        ...
                        <div class="relative w-full">
                            ...
                            <input type="text" name="name" value="{{ old('name', $mahasiswa->user->name) }}" required placeholder="Masukkan nama lengkap tanpa singkatan" 
                                class="w-full border border-gray-200 bg-[#f8fafc] pl-11 pr-4 py-3 rounded-xl text-xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 focus:bg-white transition text-gray-700 font-medium shadow-sm">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                        <div class="space-y-1.5">
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="text" name="nim" value="{{ old('nim', $mahasiswa->nim) }}" required placeholder="contoh: 2010412034" 
                                    class="w-full border border-gray-200 bg-[#f8fafc] pl-11 pr-4 py-3 rounded-xl text-xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 focus:bg-white transition text-gray-700 font-medium shadow-sm">
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="text" readonly value="{{ auth()->user()->dosen->prodi->nama_prodi }}" 
                                    class="w-full border border-gray-200 bg-gray-50/80 pl-11 pr-4 py-3 rounded-xl text-xs text-gray-400 font-semibold shadow-sm cursor-not-allowed">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="pt-4 flex justify-end space-x-3 border-t border-gray-100 w-full">
                ...
                <button type="submit" class="px-7 py-3 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl shadow-md shadow-blue-100 transition">
                    <i class="fa-solid fa-cloud-arrow-up mr-1.5"></i> Perbarui Data Mahasiswa
                </button>
            </div>
