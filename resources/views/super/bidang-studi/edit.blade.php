<x-app-layout>
    <x-slot name="title">Edit Bidang Studi - Admin</x-slot>

    <div class="mb-6 w-full">
        <h3 class="font-bold text-xl text-gray-900 tracking-tight">Edit Bidang Studi</h3>
        <p class="text-xs text-gray-400 mt-0.5">Ubah rincian data bidang studi</p>
    </div>

    <div class="w-full bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <form action="{{ route('super_admin.bidang-studi.update', $bidangStudi->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-4 w-full">
                <div>
                    ...
                    <select name="prodi_id" class="w-full px-3 py-2 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-50 focus:border-blue-500 transition font-medium text-gray-800 @error('prodi_id') border-rose-500 @enderror">
                        @foreach($prodis as $prodi)
                            <option value="{{ $prodi->id }}" {{ old('prodi_id', $bidangStudi->prodi_id) == $prodi->id ? 'selected' : '' }}>{{ $prodi->nama_prodi }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $bidangStudi->nama) }}" 
                           class="w-full px-3 py-2 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-50 focus:border-blue-500 transition font-medium text-gray-800 @error('nama') border-rose-500 @enderror">
